...
public class FindStateGovernor extends HttpServlet {
  ...
  public void doGet(HttpServletRequest req, HttpServletResponse resp)
    ...
    if (statename == null || statename.trim().isEmpty() || year == null || year.trim().isEmpty()) {
      messages.put("success", "Please enter a valid State name and Year.");
    } else {
      try {
        state = stateDao.getStateByName(statename);
        stategovernors = stateGovernorDao.getStateGovernorsByStateKeyAndYear(state.getStateKey(), Short.valueOf(year));
